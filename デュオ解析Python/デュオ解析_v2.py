...
def write_excel(df, heaven_rate_df, heaven_by_chain_df, crosstab_df, rule3_df, chains_df):
    ...
    with pd.ExcelWriter(OUTPUT_FILE, engine='openpyxl') as writer:
        ...
        for g in range(0, 801, 5):
            ...
            rate_r0_disp = rate_r0 if sample_r0 > 0 else "-"
            ...
            ws_ceil.cell(row=row, column=4, value=rate_r0_disp)
            if sample_r0 > 0: ws_ceil.cell(row=row, column=4).number_format = '0.00%'
            ...
            rate_oth_disp = rate_oth if sample_oth > 0 else "-"
            ...
            ws_ceil.cell(row=row, column=9, value=rate_oth_disp)
            if sample_oth > 0: ws_ceil.cell(row=row, column=9).number_format = '0.00%'
        ...
        ws_ceil.column_dimensions['E'].width = 2
        ...
        # 設定1調整係数 (Pythonで計算)
        setting1_adjust = 0.972 / payout_rate if payout_rate > 0 else 1.0
        ...
        for sheet_n in tqdm(range(9), desc="Creating Sheets"):
            ...
            for k in range(10):
                ...
                for cat in categories:
                    # カテゴリに対応する列名 (Loc_{cat})
                    col_name = f'Loc_{cat}'
                    if col_name not in chains_df.columns:
                        continue
                    
                    # 事前フィルタリング (テーブル単位)
                    # 条件: 
                    # 1. R3_Through == thr_val
                    # 2. Loc_{cat} == judge_val
                    # 3. Max_Daily_Balance_Before < 500
                    # 4. Prev_Special_Judge < 1000
                    
                    mask_table = (
                        (chains_df['R3_Through'] == thr_val) &
                        (chains_df[col_name] == judge_val) &
                        # (chains_df['Max_Daily_Balance_Before'] < 500) & # User requested removal
                        (chains_df['Prev_Special_Judge'] < 1000)
                    )
                    df_table = chains_df[mask_table]
                    
                    # 列幅調整 (7列分)
                    for col_offset in range(7):
                        ws_ev.column_dimensions[get_column_letter(current_col + col_offset)].width = [10, 12, 12, 12, 10, 10, 12][col_offset]
                    ...
                    for i, start_g in enumerate(range(35, 801, 5)):
                        ...
                        # --- Python 計算 ---
                        # サブセット: First_G >= start_g
                        # (df_tableは既に他の条件で絞り込み済み)
                        df_cell = df_table[df_table['First_G'] >= start_g]
                        sample_count = len(df_cell)
                        ...
                        if sample_count > 0:
                            # 初当たり = (平均初当G - 開始G) / setting1_adjust
                            avg_first_g = df_cell['First_G'].mean()
                            hatsu_calc = (avg_first_g - start_g) / setting1_adjust
                            hatsu_val = hatsu_calc
                            
                            # TY (First_G < 800 限定)
                            df_ty = df_cell[df_cell['First_G'] < 800]
                            if len(df_ty) > 0:
                                avg_dedama = df_ty['Total_Dedama'].mean()
                                ty_calc = avg_dedama - 32 * (50/25.3)
                                ty_val = ty_calc
                                
                                # 調整TY = TY * setting1_adjust
                                adj_ty_calc = ty_calc * setting1_adjust
                                adj_ty_val = adj_ty_calc
                                
                                # 5枚等価 = (調整TY - 初当たり * (50/25.3)) * 20
                                val_5mai = (adj_ty_calc - hatsu_calc * (50/25.3)) * 20
                                
                                # 出玉率
                                if real_dedama_per_g > 0:
                                    numer = hatsu_calc * 3 + (adj_ty_calc / real_dedama_per_g * 3) + val_5mai / 20
                                    denom = hatsu_calc * 3 + (adj_ty_calc / real_dedama_per_g * 3)
                                    if denom != 0:
                                        val_rate = numer / denom
                                else:
                                    # Fallback or skip if real_dedama_per_g is 0 (unlikely but safe)
                                    val_rate = "-"
                        ...
                        # 2: 初当たり
                        cell = ws_ev.cell(row=row_idx, column=current_col+1, value=hatsu_val)
                        if hatsu_val != "-": cell.number_format = '0.0'
                        cell.fill = row_fill
                        cell.border = ev_border
                        
                        # 3: TY
                        cell = ws_ev.cell(row=row_idx, column=current_col+2, value=ty_val)
                        ...
                        if val_5mai != "-": cell.number_format = '#,##0'
                        cell.fill = row_fill
                        cell.border = ev_border
                        
                        # 5: 出玉率
                        cell = ws_ev.cell(row=row_idx, column=current_col+4, value=val_rate)
                        if val_rate != "-": cell.number_format = '0.0%'
                        cell.fill = row_fill
                        cell.border = ev_border
                        
                        # 6: サンプル数
                        cell_val = sample_count if sample_count > 0 else "-"
                        cell = ws_ev.cell(row=row_idx, column=current_col+5, value=cell_val)
                        if sample_count > 0: cell.number_format = '#,##0'
                        cell.fill = row_fill
                        cell.border = ev_border
                        
                        # 7: 調整TY
                        cell = ws_ev.cell(row=row_idx, column=current_col+6, value=adj_ty_val)
                        if adj_ty_val != "-": cell.number_format = '#,##0'
                        cell.fill = row_fill
                        cell.border = ev_border
        ...
        if 'Loc_全体' in chains_df.columns:
            ...
            mask_no2 = (chains_df['Loc_全体'] == '2連なし') & (chains_df['Prev_Special_Judge'] < 1000) # User requested removal of MaxDaily filter
        ...
        for k in range(10):
            ...
            for i, start_g in enumerate(range(35, 801, 5)):
                ...
                if total_cnt > 0:
                    ...
                    hatsu_calc = (avg_g - start_g) / setting1_adjust
                    hatsu_val = hatsu_calc
                    ...
                    if total_cnt_ty > 0:
                         ...
                         # 出玉率
                         numer = hatsu_calc * 3 + (adj_ty_calc / 3.1 * 3) + val_5mai / 20
                         denom = hatsu_calc * 3 + (adj_ty_calc / 3.1 * 3)
                         if denom != 0:
                             val_rate = numer / denom
                ...
                # 2列目: 初当たり
                cell = ws_ev_no2.cell(row=row_idx, column=current_col_no2 + 1, value=hatsu_val)
                if hatsu_val != "[-]": cell.number_format = '0.0'
                cell.fill = row_fill
                cell.border = ev_border
                ...
                # 4列目: 5枚等価
                cell = ws_ev_no2.cell(row=row_idx, column=current_col_no2 + 3, value=val_5mai)
                if val_5mai != "[-]": cell.number_format = '#,##0'
                cell.fill = row_fill
                cell.border = ev_border
                
                # 5列目: 出玉率
                cell = ws_ev_no2.cell(row=row_idx, column=current_col_no2 + 4, value=val_rate)
                if val_rate != "[-]": cell.number_format = '0.0%'
                cell.fill = row_fill
                cell.border = ev_border
                ...
                # 7列目: 調整TY
                cell = ws_ev_no2.cell(row=row_idx, column=current_col_no2 + 6, value=adj_ty_val)
                if adj_ty_val != "[-]": cell.number_format = '#,##0'
                cell.fill = row_fill
                cell.border = ev_border
        ...
        # ========== test sheet start ==========
        # ユーザー要望: 0スルー目2連発生期待値表一覧の「2連目1G_RB」のみ、別シートに数式で出力
        ws_test = writer.book.create_sheet("Test_Formula_0_2Ren1GRB")
        ws_test['A1'] = "【数式検証用】0スルー目2連発生・3連基準2スルー・2連目1G_RB"
        ws_test['A1'].font = Font(bold=True, size=14)
        
        # 条件定義
        # シート: 0スルー目2連発生 -> sheet_n=0
        # テーブル: 2連発生0スルー目 -> k=0
        # -> R3_Through = 0 + 0 + 2 = 2
        # -> Loc_2連目1G_RB = '0スルー目'
        
        test_thr_val = 2
        test_judge_val = "0スルー目"
        test_cat = "2連目1G_RB"
        
        # 列参照 (ChainData)
        col_r3_thr_let = col_map.get('3連基準スルー')
        col_cat_judge_let = col_map.get(f'判定_{test_cat}') # Loc_2連目1G_RB -> 判定_2連目1G_RB
        col_prev_spec_let = col_map.get('前回特殊判定')
        col_first_g_let = col_map.get('初当G')
        col_total_dedama_let = col_map.get('純増枚数')
        
        # 範囲参照
        ref_r3_thr = f"ChainData!${col_r3_thr_let}$2:${col_r3_thr_let}${chain_max_row}"
        ref_cat_judge = f"ChainData!${col_cat_judge_let}$2:${col_cat_judge_let}${chain_max_row}"
        ref_prev_spec = f"ChainData!${col_prev_spec_let}$2:${col_prev_spec_let}${chain_max_row}"
        ref_first_g = f"ChainData!${col_first_g_let}$2:${col_first_g_let}${chain_max_row}"
        ref_total_dedama = f"ChainData!${col_total_dedama_let}$2:${col_total_dedama_let}${chain_max_row}"
        
        # 共通条件: R3_Through=2, Loc=0スルー目, PrevSpec<1000
        # (MaxDaily除外済み)
        base_cond = f'{ref_r3_thr}, {test_thr_val}, {ref_cat_judge}, "{test_judge_val}", {ref_prev_spec}, "<1000"'
        
        # ヘッダー作成
        headers_test = ["開始G", "初当たり", "TY", "5枚等価", "出玉率", "サンプル数", "調整TY"]
        for i, h in enumerate(headers_test):
            cell = ws_test.cell(row=3, column=1 + i, value=h)
            cell.fill = ev_header_fill
            cell.font = ev_header_font
            cell.border = ev_border
            ws_test.column_dimensions[get_column_letter(1 + i)].width = 12
            
        # データ行
        start_row_test = 4
        for i, g in enumerate(range(35, 801, 5)):
            row = start_row_test + i
            
            # 1. 開始G
            ws_test.cell(row=row, column=1, value=g).border = ev_border
            
            # Excel自体のセル参照 (計算用)
            addr_sample = f"F{row}"
            addr_hatsu = f"B{row}" # 初当たり
            addr_ty = f"C{row}"    # TY
            addr_adj = f"G{row}"   # 調整TY
            
            # 6. サンプル数
            # COUNTIFS(..., StartG >= g)
            f_sample = f'=COUNTIFS({base_cond}, {ref_first_g}, ">={g}")'
            ws_test.cell(row=row, column=6, value=f_sample).border = ev_border
            
            # 2. 初当たり
            # (AVERAGEIFS(...) - g) / Setting1_Adjust(CeilingAnalysis!M6)
            f_avg_g = f'AVERAGEIFS({ref_first_g}, {base_cond}, {ref_first_g}, ">={g}")'
            f_hatsu = f'=IF({addr_sample}=0, "[-]", ({f_avg_g} - {g}) / CeilingAnalysis!$M$6)'
            ws_test.cell(row=row, column=2, value=f_hatsu)
            ws_test.cell(row=row, column=2).number_format = '0.0'
            ws_test.cell(row=row, column=2).border = ev_border
            
            # 3. TY
            # AVERAGEIFS(Dedama, ..., StartG < 800) - Cost
            # Cost = 32 * (50 / COIN_HOLD)
            cost_val = 32 * (50 / 25.3) # 63.24...
            f_avg_dedama = f'AVERAGEIFS({ref_total_dedama}, {base_cond}, {ref_first_g}, ">={g}", {ref_first_g}, "<800")'
            # 件数チェック必要 (StartG < 800 の件数が0ならエラーになるため IFERROR推奨)
            f_ty = f'=IFERROR({f_avg_dedama} - {cost_val}, "[-]")'
            ws_test.cell(row=row, column=3, value=f_ty)
            ws_test.cell(row=row, column=3).number_format = '#,##0'
            ws_test.cell(row=row, column=3).border = ev_border
            
            # 7. 調整TY
            # TY * Setting1_Adjust
            f_adj = f'=IF({addr_ty}="[-]", "[-]", {addr_ty} * CeilingAnalysis!$M$6)'
            ws_test.cell(row=row, column=7, value=f_adj)
            ws_test.cell(row=row, column=7).number_format = '#,##0'
            ws_test.cell(row=row, column=7).border = ev_border
            
            # 4. 5枚等価
            # (AdjTY - Hatsu * (50/25.3)) * 20
            # 50/25.3 = 1.976...
            coin_cost = 50 / 25.3
            f_5mai = f'=IF(OR({addr_adj}="[-]", {addr_hatsu}="[-]"), "[-]", ({addr_adj} - {addr_hatsu} * {coin_cost}) * 20)'
            ws_test.cell(row=row, column=4, value=f_5mai)
            ws_test.cell(row=row, column=4).number_format = '#,##0'
            ws_test.cell(row=row, column=4).border = ev_border
            
            # 5. 出玉率
            # Denom = Hatsu*3 + AdjTY/3.1*3
            # Numer = Denom + 5mai/20
            f_denom = f'({addr_hatsu}*3 + {addr_adj}/3.1*3)'
            f_numer = f'({f_denom} + {ws_test.cell(row=row, column=4).coordinate}/20)'
            f_rate = f'=IF(OR({addr_hatsu}="[-]", {addr_adj}="[-]", {f_denom}=0), "[-]", {f_numer} / {f_denom})'
            ws_test.cell(row=row, column=5, value=f_rate)
            ws_test.cell(row=row, column=5).number_format = '0.0%'
            ws_test.cell(row=row, column=5).border = ev_border
